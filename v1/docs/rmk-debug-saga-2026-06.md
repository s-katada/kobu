...
## Phase 5: マウス操作(クリック)の救済(6/12 夕方、別セッション) `d80002d`

### 症状(2件)
1. **「ポインタをほとんど静止した状態じゃないとちゃんとクリック出来ない」** — 動きながらクリックするとカーソルが跳んで狙いが外れる
2. **「静止するために微調整しているとオートマウスレイヤーが切れちゃってクリックできなくなる」** — 止めて狙っている間にレイヤー4が消え、クリックのつもりが文字打鍵になる

### 原因
1. CLICK_FREEZE(40ms)は押下時の玉の機械的ブレを**pend_\*に貯金**し、freeze明けに**1レポートで一括放出**していた → ホスト視点では mousedown→ジャンプ→mouseup の**マイクロドラッグ**(クリック判定が外れる)
2. アイドルタイムアウト150msが、人間の「静止→狙う→指をボタンへ動かす」の所要(約200〜400ms)より**短かった**。皮肉なことに、150msに切り詰めた当時の定数コメントの⚠️警告が**この失敗を正確に予言していた**

### 修正(`d80002d`)
1. **クリック運動量の破棄**: freeze 40→80ms(玉の機械的セトリングをカバー)+貯金ではなく**破棄**(press edgeでpend_\*をゼロ化)。押してからのドラッグは無傷(掴み位置=押下時点)
2. **オートマウスホールドの文脈対応再設計**:
   - アイドル窓 600ms(クリック準備に十分な幅)→ 後日「有効時間が長い」とのフィードバックで 400ms(`ad4c7e4`)→**300ms** に追い込み。即キルが効かないのは**レイヤー4の非透過位置**(Y/U/I/O のマウスボタン等)だけなので、窓の長さ=「マウス直後のタイプが誤クリックに化ける」露出時間。300msは人間の静止→照準→クリック(約200〜400ms)の下限ギリギリ＝これ以上削ると150ms時代のクリック不能が再発する床
   - **再アーム条件**: 動き/マウスボタンのエッジ(ダブルクリック連鎖が生存)/ボタン保持(ドラッグは無期限)/非タイピングキー押下(レイヤー4のCmd+C/V/Tabチョードや単独修飾でレイヤーが落ちない)
   - **タイピング即キル**: 「150msの短さ」が本来担っていた“すぐタイピングに戻れる”は、タイムアウトではなく**即時検出**に置き換え — 新レジストリパッチ `patch_rmk_typing_tick`(keymap.rs `get_action_with_layer_cache` で、**透過なレイヤー4スロットを下の層へフォールスルーした非修飾キー押下**=本物のタイピング、を `KOBU_LAST_TYPING_TICKS` に刻印)+25msポーリングのホールドループが1ポーリング以内にレイヤーを落とす
   - 設計判断: レイヤー4は左手側が高密度(Cmd+C/V/Tab等)なので「どのキー押下でも殺す」は誤り。「**透過フォールスルー=タイピング**」が正しい判別子
   - 再開1打目は透過でそのまま正しく打鍵され、2打目以降はキル後のベースレイヤーに着地(25ms << 打鍵間隔80ms+)

### 結果
ユーザー確認「**かなり良くなった**」。レビュー所見: パッチ対象(battery.rs/keymap.rs)は既存パッチ群(keyboard.rs/morse.rs系)とアンカー非干渉、マーカー冪等の規律も維持、`cargo clean -p rmk` 実施済み・全4 UF2再生成済みで健全。

### 追補(同日のフォローアップ、ユーザー確認「good」)
- **レイヤー4左手の全透過化**(ユーザー要望): 旧レイヤー4左手の Cmd+[ / Cmd+] / TD(2) Shift / Tab+LGui / Ctrl / Cmd+C/V クラスタを撤去し、左手3行を全部 `_` に(親指行は元々透過)。これで「透過フォールスルー=タイピング」の即キル判定が**左手全域**に効く — マウス操作直後でも左手1打目から正常打鍵+レイヤー即落ち。右手の MouseBtn1/2/2/3 だけレイヤー4に残存(マウス中のクリックはレイヤーで解決されるのでキルされない)。TD(2)は未参照になったが TD(0)/TD(1) のインデックス維持のため定義は残置。キーマップ変更なので反映には clearlayout 儀式が必要
- **アイドル窓の追い込み**: 600→400→**300ms**(経緯は上記「修正」の項)
...
9. **定数コメントの⚠️警告は予言**: AUTO_MOUSE_TIMEOUT 150ms化のときに書いた「stop→aim→clickでレイヤーが落ちる」警告が3日後にそのまま現実化した。トレードオフを切り詰めるときは、警告コメントを残すだけでなく代替手段(今回はタイピング即キル)とセットで設計する
10. **盲撃ちの前に物理切り分け**: 「USB+BT-OFF」「電源直後のKarabiner未把握ウィンドウ」「Karabiner-EventViewer」のようなゼロコスト観測が、6連敗ぶんの試行錯誤より速い
...
| Cmdサム | 純LGui(tap無し) |
| Flow Tap | 廃止 |
| クリック(動きながら) | 押下エッジで運動量破棄(80ms)— マイクロドラッグ消滅 |
| オートマウスレイヤー | アイドル300ms+クリック/ドラッグ/チョードで再アーム+タイピングで即キル(≤25ms) |
| Karabiner | `;`/`:`交換・Ctrl単体・Cmd単体の3ルールからkobuを除外(他デバイスは従来どおり) |

主要な独自パッチ(build.rs内、すべてマーカー冪等): r25再アサート / PPCP / split CE長+予約 / split間隔8.75ms / SDCキュー8 / shift-chord press-edge(v3+v5) / かな救済(休眠) / kana press-edge / enter press-edge / normal-modeバッファリング / flow-tap除外群 / clearlayout完全リシンク / typing-tick(タイピング即キル)
